...
class PerturbationsProgram(FlpzCore):
    ...
    """
    def __init__(self, input_file, smodes_input, target_irrep, smodes_path="/home/iperez/isobyu/smodes", piezo=False):
        ...
        if len(normalized_phonon_vecs) == 0:
            ...
            for pert in normalized_phonon_vecs:
                perturbations = Perturbations(abinit_file=self.genstruc, min_amp=self.min_amp,
                                            max_amp=self.max_amp, perturbation=pert, batch_script_header_file=self.batch_header_file, piezo=False)
